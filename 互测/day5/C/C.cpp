...
poly calc(int n, int L, int R) {
  poly res(n + 1);

  if (R - L < 1000) {
    ++L, --R;
    std::vector<mint> f(R - L + 1);
    f[-L] = 1;
    res[0] = 1;
    for (int i = 1; i <= n; ++i) {
      std::vector<mint> g(R - L + 1);
      for (int j = 0; j <= R - L; ++j) {
        if (j - 1 >= 0) {
          g[j] += f[j - 1];
        }
      ...
      f.swap(g);
      res[i] = std::accumulate(f.begin(), f.end(), mint());
    }
  } else {
    auto add = [&](int l, int r, int op) {
      binom_prefix f, g;
      for (int i = 0; i <= n; ++i) {
        mint v;
        if (r + i >= 0) {
...
int main() {
  ...
  int d, A, B, C, a, b, c;
  std::cin >> d >> A >> B >> C >> a >> b >> c;

  init(d);
  auto fA = calc(d, -a, A - a + 1);
  auto fB = calc(d, -b, B - b + 1);
  auto fC = calc(d, -c, C - c + 1);
