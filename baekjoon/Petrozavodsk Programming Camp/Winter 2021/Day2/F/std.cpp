...
int main() {
  ...
  while (T--) {
    int n;
    std::cin >> n;
    std::vector<geo::point> p(n);
    for (int i = 0; i < n; ++i) {
      std::cin >> p[i];
    }
    std::vector<geo::line> l;
    for (int i = 1; i < n; ++i) {
      l.emplace_back(geo::line(p[0], p[i]).vertical_bisector());
    }
    geo::point min(-1e18, -1e18), max(1e18, 1e18);
    auto a = geo::half_plane_intersection_id(l, min, max);
