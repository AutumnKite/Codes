...
class seg_tree {
  ...
  std::pair<int, node> find(tnode *u, tnode *v, int l, int r,
    ...
    if (val[r - 1] < L || !(tu.v - tv.v).c) {
      return std::make_pair(std::min(val[r], R), tu.v - tv.v);
    }
    if (l + 1 == r) {
      return std::make_pair(val[l], node());
    }
    int mid = (l + r) >> 1;
    auto p = find(tu.ls, tv.ls, l, mid, L, R);
    if (val[mid] < R && p.first == val[mid]) {
...
public:
  ...
  std::pair<int, node> find(const seg_tree &rhs,
    ...
    L = std::min(L, R);
    return find(rt, rhs.rt, 0, cnt, L, R);
  }
};
