...
#define N 2050
#define P 998244353
int n, p, m, pw[35], dp[35][55][N], s[55][N], s1[55][N], tmp[N];
	...
	for (register int i = 0; i < 30; ++i)
		for (register int j1 = 0; j1 < p; ++j1)
			for (register int j2 = 0; j2 < p; ++j2){
				multiply(m + 1, dp[i][j1], m + 1, dp[i][j2], tmp);
				for (register int k = 0; k <= m; ++k)
					upd(dp[i + 1][(1ll * j1 * pw[i] + j2) % p][k], tmp[k]);
			}
	s[0][0] = 1;
	for (register int i = 30; ~i; --i)
		if ((1 << i) <= n){
			for (register int j = 0; j < p; ++j)
				for (register int k = 0; k <= m; ++k)
					s1[j][k] = s[j][k], s[j][k] = 0;
			for (register int j1 = 0; j1 < p; ++j1)
				for (register int j2 = 0; j2 < p; ++j2){
					multiply(m + 1, s1[j1], m + 1, dp[i][j2], tmp);
					for (register int k = 0; k <= m; ++k)
						upd(s[(1ll * j1 * pw[i] + j2) % p][k], tmp[k]);
				}
			n -= (1 << i);
		}
