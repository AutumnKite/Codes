...
namespace Query{
	...
	int ans;
	void init(){
		A = read(), B = read(), C = read(), P = read(), lastAns = 0;
	}
	int rnd(){
	...
	void set_ans(long long _ans){
		lastAns = _ans, ans = (ans + _ans) % 1000000007;
	}
	void print_ans(){
...
int main(){
	...
	while (q--){
		int l, r, p;
		Query::get(l, r), p = query(l, r);
		Query::set_ans(1ll * a[p] * (p - l + 1) * (r - p + 1) + sf[r] - sf[p] - f[p] * (r - p) + sg[l] - sg[p] - g[p] * (p - l));
	}
	Query::print_ans();
}