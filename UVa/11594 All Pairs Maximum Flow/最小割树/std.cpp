...
struct Graph{
	...
	void pre(){ edge = 0, memset(hd, -1, sizeof hd); }
...
struct Gomory_Hu_Tree{
	...
	void pre(){ edge = 0, memset(hd, 0, sizeof hd); }
...
int main(){
	int _T = read();
	for (register int _ = 1; _ <= _T; ++_){
		G.pre(), T.pre();
		n = read();
		for (register int i = 1; i <= n; ++i)
			for (register int j = 1, w; j <= n; ++j)
				w = read(), i < j ? G.addedge(i, j, w) : void(0);
		std :: vector<int> A;
		for (register int i = 1; i <= n; ++i) A.push_back(i);
		T.build(A);
		T.dfs(1);
		for (register int i = 1; i <= n; ++i)
			for (register int j = i + 1; j <= n; ++j)
				dis[i][j] = dis[j][i] = T.query(i, j);
		printf("Case #%d:\n", _);
		for (register int i = 1; i <= n; ++i)
			for (register int j = 1; j <= n; ++j)
				printf("%d", dis[i][j]), putchar(" \n"[j == n]);
	}
}
