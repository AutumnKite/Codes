...
template<typename T>
using min_seg_tree = seg_tree<T, min<T>>;
...
int main() {
  ...
  while (T--) {
    ...
    min_seg_tree<std::pair<int, int>> T(a.begin(), a.end());
    int q;
    std::cin >> q;
    while (q--) {
      int l, r;
      ...
      --l;
      std::vector<int> pos;
      while ((int)pos.size() < r - l && (int)pos.size() < 31) {
        int p = T.query(l, r).second;
        pos.push_back(p);
        T.modify(p, std::make_pair(1 << 30, p));
      }
      int ans = 1 << 30;
      for (int i : pos) {
        for (int j : pos) {
          if (i != j) {
            ans = std::min(ans, a[i].first | a[j].first);
          }
        }
      }
      for (int p : pos) {
