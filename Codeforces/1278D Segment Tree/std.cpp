...
void solve(){
	...
	for (register int i = 1; i <= n; ++i){
		auto l = S.lower_bound(std::make_pair(a[i].l, 0)), r = S.lower_bound(std::make_pair(a[i].r, 0));
		for (; l != r; ++l)
			if (!D.merge(i, l->second)) return prints("No"), void(0);
			else ++cnt;
		S.insert(std::make_pair(a[i].r, i));
	}
	if (cnt != n - 1) return prints("No"), void(0);
	prints("Yes");
}
int main(){
#ifdef AT_HOME
