...
void solve(){
	...
	for (register int i = 2; i <= n; ++i){
		...
		for (register int j = L; j <= m; ++j){
			while (h <= t && f[i - 1][Q[t]] - a[i][Q[t] - L] <= f[i - 1][j] - a[i][j - L]) --t;
			Q[++t] = j;
			if (Q[h] <= j - L) ++h;
			f[i][j] = std::max(f[i][j], f[i - 1][Q[h]] - a[i][Q[h] - L] + a[i][j]);
		}
		h = 1, t = 0;
		for (register int j = m; j >= L; --j){
			while (h <= t && f[i - 1][Q[t]] + a[i][Q[t]] <= f[i - 1][j] + a[i][j]) --t;
			Q[++t] = j;
			if (Q[h] >= j + L) ++h;
			f[i][j] = std::max(f[i][j], f[i - 1][Q[h]] + a[i][Q[h]] - a[i][j - L]);
		}
	}
	int ans = 0;
