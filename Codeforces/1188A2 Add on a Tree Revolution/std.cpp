...
int main() {
	...
	for (int i = 0; i < n; ++i) {
		if ((int)E[i].size() == 2 && E[i][0].second != E[i][1].second) {
			std::cout << "NO\n";
			return 0;
		}
	}
	...
	std::function<std::pair<int, int>(int, int, int, int)> dfs = [&](int u, int fa, int fw, int rt) {
		std::vector<int> leaf;
		int sum = 0;
		for (auto [v, w] : E[u]) {
			if (v != fa) {
				auto tmp = dfs(v, u, w, rt);
				leaf.push_back(tmp.first);
				sum += tmp.second;
			}
		}
		if (leaf.empty()) {
			ans.emplace_back(u, rt, fw);
			return std::make_pair(u, fw);
		}
		if ((int)leaf.size() == 1) {
			return std::make_pair(leaf[0], sum);
		}
		int t = (fw - sum) / 2;
		ans.emplace_back(leaf[0], rt, t);
		ans.emplace_back(leaf[1], rt, t);
		ans.emplace_back(leaf[0], leaf[1], -t);
		return std::make_pair(leaf[0], sum + 2 * t);
	};

	int rt = 0;
	while (rt < n && (int)E[rt].size() != 1) {
		++rt;
	}
	dfs(rt, -1, 0, rt);
