...
int main() {
  ...
  while (T--) {
    int m = 3;
    std::vector<geo::circle> c(m);
    for (int i = 0; i < m; ++i) {
      std::cin >> c[i].o.x >> c[i].o.y >> c[i].r;
    }
    std::vector<geo::point> a;
    for (int i = 0; i < m; ++i) {
      for (int j = 0; j < 36; ++j) {
        a.emplace_back(c[i].o.x + cos(j / 18.0 * geo::pi) * c[i].r,
                       c[i].o.y + sin(j / 18.0 * geo::pi) * c[i].r);
      }
    }
    a = geo::convex_hull(a);
    double ans = geo::polygon_directed_area(a);
    for (std::size_t i = 0; i < a.size(); ++i) {
      geo::line l(a[i], a[(i + 1) % a.size()]);
      for (int j = 0; j < m; ++j) {
        auto tmp = geo::circle_line_cross(c[j], l);
        if (tmp.size() == 2 && tmp[0].sgn_equal(l.a) && tmp[1].sgn_equal(l.b)) {
          double th = geo::angle(tmp[0] - c[j].o, tmp[1] - c[j].o);
          if (geo::sgn(th) < 0) {
            th += 2 * geo::pi;
          }
          ans += th * c[j].r * c[j].r / 2;
          ans -= geo::cross(tmp[0] - c[j].o, tmp[1] - c[j].o) / 2;
        }
      }
    }
    std::cout << ans << "\n";
