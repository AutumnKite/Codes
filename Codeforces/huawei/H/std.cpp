...
public:
  ...
  void exp_solve(poly &g, const poly &f, size_type l, size_type r) {
    if (r - l <= 64) {
    ...
    for (size_type i = 1; i < EXP_BLOCKS; ++i) {
      ta[i] = poly(std::vector<mint>(f.f.begin() + (i - 1) * len,
                                     f.f.begin() + (i + 1) * len));
      ta[i].DFT(len << 1);
    }
    ...
    for (size_type i = 0; i < EXP_BLOCKS; ++i) {
      ...
      exp_solve(g, f, nl, nr);
      if (i + 1 < EXP_BLOCKS) {
        tb[i] = poly(std::vector<mint>(g.f.begin() + nl, g.f.begin() + nr));
        init.build_rev(len << 1);
        tb[i].DFT(len << 1);
...
int main() {
  ...
  std::cout << solve(1, m + 1).inv(n + 1)[n] * tot.inv() << "\n";
}
