...
int main() {
	...
	for (int i = 1; i <= m; ++i) {
		for (int j = 0; j < i; ++j) {
			int t = (1ll * j * (j + 1) / 2 + 1ll * (i - j) * (i - j - 1) / 2) % P;
			g[i] = (g[i] + 1ll * g[j] * g[i - j - 1] % P * (i + 1) % P * C[i - 1][j]) % P;
			f[i] = (f[i] + 1ll * g[j] * g[i - j - 1] % P * t % P * C[i - 1][j]) % P;
			f[i] = (f[i] + 1ll * f[j] * g[i - j - 1] % P * (i + 1) % P * C[i - 1][j]) % P;
			f[i] = (f[i] + 1ll * g[j] * f[i - j - 1] % P * (i + 1) % P * C[i - 1][j]) % P;
		}
	}

	std::vector<std::vector<int>> F(n + 2, std::vector<int>(m + 1)), G(F);
	G[0][0] = 1;
	for (int i = 1; i <= n + 1; ++i) {
		for (int j = 0; j <= m; ++j) {
			for (int k = 0; k <= j && k < i; ++k) {
				G[i][j] = (G[i][j] + 1ll * G[i - k - 1][j - k] * g[k] % P * C[j][k]) % P;
				F[i][j] = (F[i][j] + 1ll * G[i - k - 1][j - k] * f[k] % P * C[j][k]) % P;
				F[i][j] = (F[i][j] + 1ll * F[i - k - 1][j - k] * g[k] % P * C[j][k]) % P;
			}
		}
	}
	std::cout << F[n + 1][m] << "\n";
