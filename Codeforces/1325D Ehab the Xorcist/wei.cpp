...
void solve(){
	...
	for (register int i = 60; ~i; --i)
		if (!now){
			...
			if (_x && _y) a |= 1ll << i;
			if (!_x && _y) now = 1;
			if (_x && !_y) a |= 1ll << i, now = 1;
		}
		else{
			int _x = x >> i & 1, _y = y >> i & 1;
			if (!_x && !_y) a |= 1ll << i, b |= 1ll << i, now = 0;
			if (!_x && _y) a |= 1ll << i, b |= 1ll << i;
			if (_x && !_y) a |= 1ll << i;
			if (_x && _y) a |= 1ll << i, b |= 1ll << i, c |= 1ll << i, now = 0;
		}
	if (now) return print(-1);
	if (c) return print(3), print(a, ' '), print(b, ' '), print(c), void(0);
	if (b) return print(2), print(a, ' '), print(b), void(0);
	if (a) return print(1), print(a), void(0);
	print(0);
}
int main(){
#ifdef AT_HOME
