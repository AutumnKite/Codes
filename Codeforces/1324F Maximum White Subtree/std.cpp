...
void dfs_f(int u, int fa = 0){
	f[u] = a[u];
	for (int v : E[u]) if (v != fa) dfs_f(v, u), f[u] += std::max(f[v], 0);
}
void dfs_g(int u, int fa = 0){
	for (int v : E[u]) if (v != fa) g[v] = std::max(g[u], 0) + f[u] - std::max(f[v], 0), dfs_g(v, u);
}
void solve(){
	read(n);
	for (register int i = 1; i <= n; ++i) read(a[i]), a[i] = a[i] ? 1 : -1;
	for (register int i = 1, u, v; i < n; ++i)
		read(u), read(v), E[u].push_back(v), E[v].push_back(u);
	dfs_f(1), dfs_g(1);
	for (register int i = 1; i <= n; ++i) print(f[i] + std::max(g[i], 0), " \n"[i == n]);
}
int main(){
#ifdef AT_HOME
