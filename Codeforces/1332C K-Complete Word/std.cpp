...
void solve(){
	...
	for (register int i = 0; i < (k + 1) / 2; ++i){
		for (register int j = 0; j < 26; ++j) c[j] = 0;
		for (register int j = i; j < n; j += k) ++c[s[j] - 'a'];
		for (register int j = k - i - 1; j < n; j += k) ++c[s[j] - 'a'];
		char mx = std::max_element(c, c + 26) - c;
		for (register int j = i; j < n; j += k) a[j] = mx + 'a';
		for (register int j = k - i - 1; j < n; j += k) a[j] = mx + 'a';
	}
	int ans = 0;
	for (register int i = 0; i < n; ++i) ans += a[i] != s[i];
	// debug("%s\n", a);
	print(ans);
}
int main(){
