...
int main() {
  ...
  while (T--) {
    ...
    std::vector<geo::line> r;
    for (int i = 0; i < n; ++i) {
      geo::point A = a[i], B = a[(i + 1) % n];
      geo::point d = (B - A).rotate90().unit() * w;
      A += d, B += d;
      r.emplace_back(A, B);
    }
    auto b = geo::half_plane_intersection(r, geo::point(-1e18, -1e18),
                                             geo::point(1e18, 1e18));
    ...
    bool ok = true;
    for (int i = 0; i < n; ++i) {
      geo::point A = b[i], B = b[(i + 1) % n];
      ok &= geo::sgn(geo::cross(A - M, B - M)) >= 0;
    }

    if (ok) {
      std::cout << geo::polygon_directed_area(b) << "\n";
      continue;
    }
    ...
    for (int i = 0; i < n; ++i) {
      geo::point A = b[i], B = b[(i + 1) % n];
      l.emplace_back(A, B);
      l.emplace_back(M + (A - M) / (z - h) * z, M + (B - M) / (z - h) * z);
    }
    auto res = geo::half_plane_intersection(l, geo::point(-1e18, -1e18),
                                               geo::point(1e18, 1e18));
    std::cout << geo::polygon_directed_area(res) << "\n";
  }
}
