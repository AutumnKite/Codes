...
int main() {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = i + 1; j < n; ++j) {
      double d = geo::distance(c[i].o, c[j].o);
      if (geo::sgn(d, c[j].r - c[i].r) <= 0) {
        seg.emplace_back(0, 2 * geo::pi);
      } else {
        auto tmp = geo::circle_cross(c[i], c[j]);
        if (tmp.size() < 2) {
          continue;
        }
        double l = (tmp[0] - c[i].o).angle();
        double r = (tmp[1] - c[i].o).angle();
        if (l < r) {
          seg.emplace_back(l, r);
        } else {
          seg.emplace_back(l, 2 * geo::pi);
