...
using geo = geometry<i64, std::less<i64>>;
...
int main() {
  ...
  auto cmp = [&](geo::point a, geo::point b) {
    i64 tmp = geo::cross(a, b);
    if (tmp != 0) {
      return tmp > 0;
    }
    return geo::dot(a, b) > 0;
  };

  auto equal = [&](geo::point a, geo::point b) {
    return geo::cross(a, b) == 0 && geo::dot(a, b) > 0;
  };

  geo::point L = (a[0] - a[1]).rotate90(), R = (a[1] - a[0]).rotate90();
  bool g = false;
  for (int i = 1; i < n - 1; ++i) {
    geo::point A = a[i], B = a[i + 1];
    geo::point p = (A - B).rotate90();
    if (equal(p, L) && equal(-p, R)) {
      continue;
    }
    if (equal(p, R) && equal(-p, L)) {
      g = true;
      continue;
    }
    if (g) {
