...
bool check(int x){
	...
	for (register int i = 1; i < (int)pos.size(); ++i){
		if (pos[i] - pos[i - 1] < x) return 0;
		for (register int j = pos[i - 1] + 1; j < pos[i]; ++j)
			if (j - pos[i - 1] < x || pos[i] - j < x) vis[j] = 1;
	}
	if (pos.size()){
		for (register int i = 1; i < pos[0]; ++i)
			if (pos[0] - i < x) vis[i] = 1;
		for (register int i = pos.back() + 1; i <= n; ++i)
			if (i - pos.back() < x) vis[i] = 1;
	}
	std::vector<int> f;
	for (register int i = 1; i <= n; ++i)
		if (s[i] == '0' && !vis[i]) f.push_back(i);
	return (int)f.size() >= 2 && f.back() - f[0] >= x;
}
int main(){
