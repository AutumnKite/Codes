...
const int N = 2005, P = 1000000007, Inv2 = (P + 1) >> 1;
int n;
char a[N][N];
int f[N][N][2];
int main(){
	...
	read(n);
	for (register int i = 1; i <= n; ++i) reads(a[i] + 1);
	int pw = 1;
	for (register int i = 1; i <= n; ++i)
		for (register int j = 1; j <= n; ++j)
			if (a[i][j] == '.') pw = 2ll * pw % P;
	for (register int i = 1; i <= n; ++i) f[0][i][0] = pw;
	for (register int i = 1; i <= n; ++i) f[i][0][1] = pw;
	for (register int i = 1; i <= n; ++i)
		for (register int j = 1; j <= n; ++j){
			f[i][j][0] = f[i][j - 1][0], f[i][j][1] = f[i - 1][j][1];
			if (a[i][j] == 'W') continue;
			f[i][j][0] = (f[i][j][0] + 1ll * f[i][j - 1][1] * Inv2) % P;
			f[i][j][1] = (f[i][j][1] + 1ll * f[i - 1][j][0] * Inv2) % P;
		}
	print((f[n][n][0] + f[n][n][1]) % P);
}
