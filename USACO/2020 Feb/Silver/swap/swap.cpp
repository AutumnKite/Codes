...
struct per{
	...
	per operator * (const per &a) const {
		per res;
		for (register int i = 1; i <= n; ++i) res.p[i] = p[a.p[i]];
		return res;
	}
}a;
per qpow(per a, int b){
	per s;
	for (s.init(); b; b >>= 1, a = a * a) if (b & 1) s = s * a;
	return s;
}
int main(){
	freopen("swap.in", "r", stdin);
	freopen("swap.out", "w", stdout);
	read(n), read(m), read(k), a.init();
	for (register int i = 1; i <= m; ++i)
		read(l), read(r), std::reverse(a.p + l, a.p + r + 1);
	a = qpow(a, k);
	for (register int i = 1; i <= n; ++i) print(a.p[i]);
}