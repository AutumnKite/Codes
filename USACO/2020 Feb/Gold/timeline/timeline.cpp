...
const int N = 100005;
int n, m, lim, d[N], h, t, Q[N], dis[N];
std::vector< std::pair<int, int> > E[N];
void topo(){
	h = t = 0;
	for (register int i = 0; i <= n; ++i)
		if (!d[i]) Q[++t] = i, dis[i] = 0;
	while (h < t){
		int u = Q[++h];
		for (auto p : E[u]){
...
int main(){
	...
	read(n), read(lim), read(m);
	for (register int i = 1, w; i <= n; ++i)
		read(w), E[0].push_back({i, w}), ++d[i];
	for (register int i = 1, u, v, w; i <= m; ++i)
		read(u), read(v), read(w), E[u].push_back({v, w}), ++d[v];
	topo();
	for (register int i = 1; i <= n; ++i) print(dis[i]);
}