...
const int N = 100005, M = 200005, P = 1000000007;
int n, m, l[N], r[N], a[M], pw[N];
void inc(int &a, int b){ (a += b) >= P ? a -= P : 0; }
void dec(int &a, int b){ a < b ? a += P - b : a -= b; }
int solve(){
	for (register int i = 1; i <= m; ++i) a[i] = 0;
	for (register int i = 1; i <= n; ++i) ++a[l[i]], --a[r[i]];
	int res = 0;
	for (register int i = 1; i <= m; ++i)
		a[i] += a[i - 1], inc(res, pw[n]), dec(res, pw[n - a[i]]);
	return res;
}
int main(){
	...
	pw[0] = 1;
	for (register int i = 1; i <= n; ++i) pw[i] = 2ll * pw[i - 1] % P;
	int ans = solve();
