...
int main() {
  ...
  if (geo::sgn(geo::cross(p[1] - p[0], p[2] - p[0])) < 0) {
    std::swap(p[1], p[2]);
  }

  double ans = 0;
  for (int i = 0; i < 3; ++i) {
    auto A = p[(i + 1) % 3], B = p[(i + 2) % 3], C = p[i];
    double aA = geo::angle(B - A, C - A) / 2;
    double aB = geo::angle(A - B, C - B) / 2;
    auto tA = (B - A).rotate(aA).unit();
    auto tB = (A - B).rotate(aB).unit();

    auto Q = geo::line_cross(geo::line(A, A + tA), geo::line(B, B + tB))[0];
    auto R = geo::projection(geo::line(A, B), Q);
    double l = 0, r = geo::distance(Q, R);
    for (int t = 0; t < 60; ++t) {
      double mid = (l + r) / 2;
      auto pA = A + tA * (mid / sin(aA));
      auto pB = B + tB * (mid / sin(-aB));
      if (geo::distance(pA, pB) >= 2 * mid) {
        l = mid;
      } else {
        r = mid;
      }
