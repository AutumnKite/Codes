...
struct BIT{
	...
	void add(int x, int y){
		for (; x; x ^= x & -x) c[x] = std::max(c[x], y);
	}
	int query(int x){
		int s = 0;
		for (; x <= m; x += x & -x) s = std::max(s, c[x]);
		return s;
	}
}T;
std::vector<int> E[N];
...
void dfs(int u){
	f[u] = 1;
	for (int v : E[u]) dfs(v), f[u] = 1ll * f[u] * (f[v] + 1) % P;
}
void solve(){
	read(n);
	for (register int i = 1; i <= n; ++i)
		read(a[i].l), read(a[i].r), a[i].r += a[i].l, x[++m] = a[i].l, x[++m] = a[i].r;
	std::sort(x + 1, x + 1 + m);
	m = std::unique(x + 1, x + 1 + m) - x - 1;
	for (register int i = 1; i <= n; ++i){
		a[i].l = std::lower_bound(x + 1, x + 1 + m, a[i].l) - x;
		a[i].r = std::lower_bound(x + 1, x + 1 + m, a[i].r) - x;
	}
	std::sort(a + 1, a + 1 + n);
	for (register int i = 1; i <= n; ++i){
		int p = T.query(a[i].l + 1);
		E[p].push_back(i), T.add(a[i].r, i);
	}
	dfs(0), print(f[0]);
}
int main(){
