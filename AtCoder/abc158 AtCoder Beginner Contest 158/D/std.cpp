...
void solve(){
	...
	while (q--){
		...
		if (op == 1) rev ^= 1;
		if (op == 2){
			read(op), --op, op ^= rev, reads(ch);
			if (op == 0) pre.push_back(ch[0]); else suf.push_back(ch[0]);
		}
	}
	if (rev) std::reverse(s + 1, s + 1 + n), std::reverse(suf.begin(), suf.end()), pre.swap(suf);
	else std::reverse(pre.begin(), pre.end());
	for (char ch : pre) putchar(ch);
	prints_(s + 1);
	for (char ch : suf) putchar(ch);
	putchar('\n');
}
int main(){
#ifdef AT_HOME
