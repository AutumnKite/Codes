...
//		if (!m) printf("%d %d\n", u, k);
	...
	void erase(int l, int r){
		int u = kth(rt, l - 1); splay(u); // print();
		int v = kth(rt, r + 1); splay(v, u); // print();
...
//		if (!u) return;
//		printf("%d: fa=%d ls=%d rs=%d sz=%d v=%d sum=%d la=%d ra=%d ma=%d rev=%d cov=%d\n", u, fa[u], son[u][0], son[u][1], sz[u], val[u], sum[u], la[u], ra[u], ma[u], rev[u], cov[u]);
//		print(son[u][0], dep + 1), print(son[u][1], dep + 1);
//	}
//	void print(){
//		printf("Rt=%d\n", rt), print(rt);
//	}
//	void update_all(int u){
//		if (!u) return;
//		down(u);
//		update_all(son[u][0]), update_all(son[u][1]);
//		up(u);
//	}
}T;
int main(){
