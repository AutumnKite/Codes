...
const int Rdx[] = {-1, 1, 0, 0}, Rdy[] = {0, 0, -1, 1}, Bdx[] = {-1, 1, -1, 1}, Bdy[] = {-1, -1, 1, 1};
char a[10][10], s[10][10];
void solve(){
	for (register int i = 1; i <= 8; ++i) reads(a[i] + 1);
		...
		for (register int j = 1; j <= 8; ++j){
			if (a[i][j] == 'R'){
				...
				for (register int k = 0; k < 4; ++k)
					for (register int x = i + Rdx[k], y = j + Rdy[k]; a[x][y] == '.'; x += Rdx[k], y += Rdy[k])
						s[x][y] = '0';
			}
			if (a[i][j] == 'B'){
				s[i][j] = '0';
				for (register int k = 0; k < 4; ++k)
					for (register int x = i + Bdx[k], y = j + Bdy[k]; a[x][y] == '.'; x += Bdx[k], y += Bdy[k])
						s[x][y] = '0';
			}
		}
	for (register int i = 1; i <= 8; ++i) prints(s[i] + 1);
}
int main(){
#ifdef AT_HOME
