...
int n;
std::vector<int> a[2025], sum;
void solve(){
	read(n);
	for (register int i = 1, x; i <= n; ++i){
		read(x);
		while (x) a[i].push_back(x % 10), x /= 10;
	}
	sum = a[1];
	for (register int i = 2; i <= n; ++i){
		int len = std::max(a[i].size(), sum.size()), x = 0;
		a[i].resize(len), sum.resize(len);
		for (register int j = 0; j < len; ++j){
			int v = a[i][j] + sum[j] + x;
			sum[j] = v % 10;
			x = v / 7;
		}
		if (x) sum.push_back(x);
	}
	if (sum.empty()) return print(0), void(0);
	while (sum.size()) putchar(sum.back() ^ '0'), sum.pop_back();
	putchar('\n');
}
int main(){
#ifdef AT_HOME
