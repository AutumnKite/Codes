...
long long rnk(long long lim){
	return lim / n * a[n] + a[lim % n];
}
long long kth(long long k){
	long long l = 1, r = k * n, md, ans = 0;
	while (l <= r) if (rnk(md = (l + r) >> 1) >= k) ans = md, r = md - 1; else l = md + 1;
	return ans;
}
void solve(){
	read(n), read(c), read(k), read(l), read(r);
	for (register int i = 1; i <= n; ++i) a[i] = a[i - 1] + (gcd(i, n) <= c);
	print(kth(k)), print(rnk(r) - rnk(l - 1));
}
int main(){
#ifdef AT_HOME
